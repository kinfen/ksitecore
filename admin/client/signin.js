...
var SigninView = React.createClass({
	...
	handleSubmit (e) {
		...
		SessionStore.signin({
			...
			if (err || data && data.error) {
				...
				console.log('success' + KSiteAdm.adminPath);
				top.location.href = KSiteAdm.adminPath;
			}
		});
	},
	...
	renderBrand () {
		let logo = { src: `${KSiteAdm.adminPath}/images/logo.png`, width: 205, height: 68 };
		if (this.props.logo) {
	...
	renderUserInfo () {
		...
		let openKeystoneButton = this.props.userCanAccessKeystone ? <Button href={KSiteAdm.adminPath} type="primary">Open KSiteAdm</Button> : null;
		return (
			<div className="auth-box__col">
				<p>Hi {this.props.user.name.first},</p>
				<p>You're already signed in.</p>
				{openKeystoneButton}
				<Button href={`${KSiteAdm.adminPath}/signout`} type="link-cancel">Sign Out</Button>
			</div>
		);
	},
	renderAlert () {
		...
		return (
			<div className="auth-wrapper">
				...
				<div className={boxClassname}>
					<h1 className="u-hidden-visually">{this.props.brand ? this.props.brand : 'KSiteAdm'} Sign In </h1>
